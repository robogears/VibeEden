...
static bool LoadNroImpl(Core::System& system, Kernel::KProcess& process,
    ...
    // Build program image. Reject a header file_size larger than the actual data, and clamp the
    // copy so a (page-aligned) image larger than the input never reads out of bounds.
    if (data.size() < nro_header.file_size) {
        return {};
    }
    std::vector<u8> program_image(PageAlignSize(nro_header.file_size));
    std::memcpy(program_image.data(), data.data(), std::min(program_image.size(), data.size()));
    ...
    // Read MOD header
    if (static_cast<u64>(nro_header.module_header_offset) + sizeof(ModHeader) >
        program_image.size()) {
        return {};
    }
    ...
    if (has_mod_header) {
        // Guard against a wrapped (bss_end < bss_start) size, which would resize to ~4 GB.
