...
std::optional<VAddr> AppLoader_NSO::LoadModule(Kernel::KProcess& process, Core::System& system, const FileSys::VfsFile& nso_file, VAddr load_base, bool should_pass_arguments, bool load_into_process, std::optional<FileSys::PatchManager> pm, std::vector<Core::NCE::Patcher>* patches, s32 patch_index) {
    ...
    // Size it to span EVERY segment, not just the last one: a crafted NSO can place an earlier
    // segment past the last segment's end, which would otherwise be an out-of-bounds heap write.
    Kernel::CodeSet codeset;
    std::size_t image_end = module_start;
    for (const auto& seg : nso_header.segments) {
        image_end = std::max<std::size_t>(
            image_end, module_start + static_cast<std::size_t>(seg.location) + seg.size);
    }
    codeset.memory.resize(image_end);
    {
        ...
        for (std::size_t i = 0; i < nso_header.segments.size(); ++i) {
            ...
            // The destination span must lie within the sized image (untrusted location/size).
            if (static_cast<u64>(module_start) + segment.location + segment.size >
                codeset.memory.size()) {
                return std::nullopt;
            }
            nso_file.Read(compressed_data.data(), nso_header.segments_compressed_size[i], segment.offset);
            if (nso_header.IsSegmentCompressed(i)) {
                int r = Common::Compression::DecompressDataLZ4(decompressed_size.data(), segment.size, compressed_data.data(), nso_header.segments_compressed_size[i]);
                if (r != static_cast<int>(segment.size)) {
                    return std::nullopt;
                }
                std::memcpy(codeset.memory.data() + module_start + segment.location, decompressed_size.data(), segment.size);
            } else {
                // Uncompressed: compressed_data only holds segments_compressed_size[i] bytes (what
                // Read filled above). A valid uncompressed segment has size == compressed size, so a
                // larger size is a crafted file trying to over-read the scratch buffer.
                if (segment.size > nso_header.segments_compressed_size[i]) {
                    return std::nullopt;
                }
                std::memcpy(codeset.memory.data() + module_start + segment.location, compressed_data.data(), segment.size);
            }
