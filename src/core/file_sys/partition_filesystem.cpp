...
PartitionFilesystem::PartitionFilesystem(VirtualFile file) {
    ...
    for (u16 i = 0; i < pfs_header.num_entries; i++) {
        ...
        // entry.strtab_offset is attacker-controlled; reject an offset outside the string table
        // before using it to index file_data (the table is NUL-terminated by the push_back above).
        if (entry.strtab_offset >= pfs_header.strtab_size) {
            status = Loader::ResultStatus::ErrorBadPFSHeader;
            return;
        }
        std::string name(
