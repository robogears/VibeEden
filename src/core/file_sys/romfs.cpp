...
VirtualDir ExtractRomFS(VirtualFile file) {
    ...
    ctx.file = file;
    // Validate the (untrusted) meta-table offsets/sizes against the file before reading, so bogus
    // header values cannot drive an oversized allocation. (Each subtraction is guarded by the
    // preceding offset-in-range clause via short-circuit.)
    const std::size_t romfs_size = file->GetSize();
    if (ctx.header.directory_meta.offset > romfs_size ||
        ctx.header.directory_meta.size > romfs_size - ctx.header.directory_meta.offset ||
        ctx.header.file_meta.offset > romfs_size ||
        ctx.header.file_meta.size > romfs_size - ctx.header.file_meta.offset) {
        return nullptr;
    }
    ctx.directory_meta =
