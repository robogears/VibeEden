...
Loader::ResultStatus ProgramMetadata::Load(VirtualFile file) {
    ...
    // Validate the (untrusted) size/offset against the file before allocating, so a bogus kac_size
    // cannot drive a multi-GB allocation (bad_alloc DoS). The first clause also catches a kac_offset
    // large enough to wrap read_offset below aci_offset.
    if (read_offset < npdm_header.aci_offset || read_offset + read_size > file->GetSize()) {
