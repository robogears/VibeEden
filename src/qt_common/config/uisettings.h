...
struct Values {
    ...
    Setting<bool> controller_applet_disabled{linkage, false, "disableControllerApplet",
                                             Category::UiGeneral};
    // Steam Deck / Steam Input: when enabled (default), connected controllers are auto-assigned to
    // players in Steam's order with a Pro Controller mapping. Disable to configure players manually.
    Setting<bool> auto_map_controllers{linkage, true, "auto_map_controllers", Category::UiGeneral};
