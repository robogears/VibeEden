...
private:
    ...
    // Steam Deck / Steam Input controller auto-mapper (binds players to Steam's controller order).
    std::unique_ptr<SteamControllerMapper> steam_controller_mapper;

#ifdef ENABLE_UPDATE_CHECKER
    QFuture<UpdateChecker::Result> update_future;
    QFutureWatcher<UpdateChecker::Result> update_watcher;
    // True when the in-flight update check was triggered manually (the status-bar Check for Updates
    // button), so the result handler knows to show explicit feedback (up-to-date / check-failed).
    bool update_check_is_manual = false;
    // True while the manual-check busy UI (wait cursor + disabled action) is active, so it is
    // restored exactly once regardless of which check completes.
    bool update_ui_busy = false;
#endif
    ...
    QPushButton* update_check_button = nullptr;
    ...
    // Periodically re-syncs controller bindings to Steam's order (hotplug detection).
    QTimer controller_map_timer;
