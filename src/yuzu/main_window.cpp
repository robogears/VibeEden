...
#include "steam_controller_mapper.h"
#include "util/util.h"
...
MainWindow::MainWindow(bool has_broken_vulkan)
    ...
    // Steam Deck / Steam Input: bind players to Steam's controller order automatically, so
    // controllers "just work" with no trip through the controller settings. Re-syncs on a timer
    // to pick up hotplugged pads. See SteamControllerMapper.
    steam_controller_mapper =
        std::make_unique<SteamControllerMapper>(*QtCommon::system, *input_subsystem);
    controller_map_timer.setInterval(1000);
    connect(&controller_map_timer, &QTimer::timeout, this,
            [this] { steam_controller_mapper->Sync(emulation_running); });
    controller_map_timer.start();
    steam_controller_mapper->Sync(emulation_running);
...
#ifdef ENABLE_UPDATE_CHECKER
    // Connect the watcher unconditionally so the manual "Check for updates" menu action works even
    // when the automatic startup check is disabled; only the startup check itself is gated.
    connect(&update_watcher, &QFutureWatcher<UpdateChecker::Result>::finished, this,
            &MainWindow::OnEmulatorUpdateAvailable);
    if (UISettings::values.check_for_updates) {
        update_check_is_manual = false;
        update_future =
            QtConcurrent::run([]() -> UpdateChecker::Result { return UpdateChecker::GetUpdate(); });
...
void MainWindow::ControllerSelectorReconfigureControllers(
    ...
    // Don't forget to apply settings. (The applet no longer enters manual configuration mode -
    // the Steam auto-mapper owns controller binding - so there is no config state to disable.)
...
#ifdef ENABLE_UPDATE_CHECKER
    // Dedicated "Check for updates" button, living in the status-bar cluster alongside the
    // renderer/dock/FSR/volume indicators (replaces the old Help-menu + settings-page entries).
    update_check_button = new QPushButton();
    update_check_button->setObjectName(QStringLiteral("TogglableStatusBarButton"));
    update_check_button->setFocusPolicy(Qt::NoFocus);
    update_check_button->setText(tr("Check for Updates"));
    update_check_button->setToolTip(tr("Check for VibeEden updates"));
    connect(update_check_button, &QPushButton::clicked, this, &MainWindow::OnCheckForUpdates);
    statusBar()->insertPermanentWidget(0, update_check_button);
#endif
...
void MainWindow::OnConfigure() {
    ...
    // Pause the Steam-order auto-map re-sync while the user edits bindings in the Advanced editor,
    // then reconcile once they close it.
    connect(&configure_dialog, &ConfigureDialog::AdvancedEditorVisibilityChanged, this,
            [this](bool open) {
                if (open) {
                    controller_map_timer.stop();
                } else {
                    steam_controller_mapper->Invalidate();
                    controller_map_timer.start();
                    steam_controller_mapper->Sync(emulation_running);
                }
            });
...
void MainWindow::OnEmulatorUpdateAvailable() {
    const bool was_manual = update_check_is_manual;
    update_check_is_manual = false;

    // Restore the manual-check busy UI exactly once (paired with OnCheckForUpdates).
    if (update_ui_busy) {
        QGuiApplication::restoreOverrideCursor();
        if (update_check_button != nullptr) {
            update_check_button->setEnabled(true);
        }
        update_ui_busy = false;
    }

    const UpdateChecker::Result result = update_future.result();
    switch (result.status) {
    case UpdateChecker::Status::UpdateAvailable:
        if (result.release) {
            UpdateDialog dialog(result.release.value(), this);
            dialog.exec();
        }
        break;
    case UpdateChecker::Status::UpToDate:
        // Only a manual check reports being up to date; the silent startup check stays silent.
        if (was_manual) {
            QtCommon::Frontend::Information(
                this, tr("No updates available"),
                tr("You are running the latest version of VibeEden."));
        }
        break;
    case UpdateChecker::Status::CheckFailed:
        // Do NOT claim "up to date" on a failed check - tell the user it could not complete.
        if (was_manual) {
            QtCommon::Frontend::Warning(
                this, tr("Update check failed"),
                tr("Could not check for updates. Please check your internet connection and try "
                   "again."));
        }
        break;
    }
...
void MainWindow::OnCheckForUpdates() {
    update_check_is_manual = true;
    if (update_future.isRunning()) {
        // A check is already in flight (e.g. the automatic startup check); let it report its
        // result now that the user has asked explicitly, rather than starting a second one.
        return;
    }
    // Give immediate feedback during the (up to ~15s) network check so the control does not look
    // dead, especially on the Deck.
    update_ui_busy = true;
    QGuiApplication::setOverrideCursor(Qt::WaitCursor);
    if (update_check_button != nullptr) {
        update_check_button->setEnabled(false);
    }
    update_future =
        QtConcurrent::run([]() -> UpdateChecker::Result { return UpdateChecker::GetUpdate(); });
    update_watcher.setFuture(update_future);
