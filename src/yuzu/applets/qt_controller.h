...
private:
    ...
    // Strips the dialog down to a simple, dark, Switch-style status screen (hides the manual
    // config widgets, adds a prompt + "Quit game" button, applies the dark theme).
    void SetupSimpleView();

    // Polls the real (Steam auto-mapped) controller state, refreshes the slot status, and
    // automatically accepts the applet once the game's required controllers are present.
    void RefreshAndMaybeAccept();

    // Whether the game's controller requirements are satisfied by the actually-connected
    // controllers (independent of the legacy manual checkboxes).
    bool AreRealParametersMet();
    ...
    // Drives the auto-resume poll for the simplified Deck-style view.
