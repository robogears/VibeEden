...
QtControllerSelectorDialog::QtControllerSelectorDialog(
    ...
    // Note: the simplified view replaces the button box (see SetupSimpleView), so the legacy
    // "accepted -> ApplyConfiguration" path is intentionally not connected: auto-resume calls
    // accept() directly and the lone "Quit game" button calls reject().
    ...
    // Replace the legacy manual layout with the simple dark Switch-style status screen.
    SetupSimpleView();
...
QtControllerSelectorDialog::~QtControllerSelectorDialog() {
    ...
    // Note: this applet no longer enters manual controller-configuration mode (the Steam
    // auto-mapper owns binding), so there is nothing to disable here.
}

int QtControllerSelectorDialog::exec() {
    // If the game's controller requirements are already satisfied by the connected controllers,
    // don't bother showing the applet at all - just resume.
    if (AreRealParametersMet() || (parameters_met && parameters.enable_single_mode)) {
        return QDialog::Accepted;
    }
    return QDialog::exec();
}

void QtControllerSelectorDialog::SetupSimpleView() {
    const auto hide_widget = [](QWidget* w) {
        if (w != nullptr) {
            w->setVisible(false);
        }
    };

    // Hide all of the legacy manual-configuration widgets, leaving only the player slots.
    for (std::size_t i = 0; i < NUM_PLAYERS; ++i) {
        hide_widget(emulated_controllers[i]);
        hide_widget(connected_controller_checkboxes[i]);
        hide_widget(connected_controller_labels[i]);
        hide_widget(connected_controller_icons[i]);
        for (auto* led : led_patterns_boxes[i]) {
            hide_widget(led);
        }
        // Turn the per-player groupbox into a plain, non-interactive slot container.
        player_groupboxes[i]->setCheckable(false);
        player_groupboxes[i]->setTitle(QString{});
    }
    hide_widget(ui->vibrationButton);
    hide_widget(ui->motionButton);
    hide_widget(ui->inputConfigButton);
    hide_widget(ui->vibrationGroup);
    hide_widget(ui->motionGroup);
    hide_widget(ui->radioDocked);
    hide_widget(ui->radioUndocked);

    // Repurpose the (normally hidden) error label as our always-visible status prompt.
    ui->labelError->setVisible(true);

    // Replace OK/Cancel with a single discreet "Quit game" (reject -> the game stops).
    ui->buttonBox->clear();
    auto* quit_button = ui->buttonBox->addButton(tr("Quit game"), QDialogButtonBox::RejectRole);
    connect(quit_button, &QPushButton::clicked, this, &QDialog::reject);

    // Simple dark theme.
    setStyleSheet(QStringLiteral(
        "QDialog { background-color: #1b1b1f; }"
        "QLabel { color: #e6e6e6; }"
        "QGroupBox { border: 1px solid #3a3a40; border-radius: 8px; margin-top: 6px; }"
        "QPushButton { background-color: #2a2a30; color: #e6e6e6; border: 1px solid #3a3a40;"
        " border-radius: 6px; padding: 6px 16px; }"
        "QPushButton:hover { background-color: #34343c; }"));
    setWindowTitle(tr("Controllers"));

    // Poll the real controller state and auto-resume the game once its needs are met.
    refresh_timer = new QTimer(this);
    refresh_timer->setInterval(250);
    connect(refresh_timer, &QTimer::timeout, this,
            &QtControllerSelectorDialog::RefreshAndMaybeAccept);
    refresh_timer->start();
    RefreshAndMaybeAccept();
}

bool QtControllerSelectorDialog::AreRealParametersMet() {
    auto& hid_core = system.HIDCore();
    const auto* handheld = hid_core.GetEmulatedController(Core::HID::NpadIdType::Handheld);

    // Clamp against a malformed (e.g. negative) guest-supplied player count before any size_t cast.
    const int min_supported_players = std::clamp<int>(
        parameters.enable_single_mode ? 1 : parameters.min_players, 0,
        static_cast<int>(NUM_PLAYERS));
    const int max_supported_players = std::clamp<int>(
        parameters.enable_single_mode ? 1 : parameters.max_players, 0,
        static_cast<int>(NUM_PLAYERS));

    int connected = 0;
    bool all_compatible = true;
    for (std::size_t i = 0; i < NUM_PLAYERS; ++i) {
        const auto* controller = hid_core.GetEmulatedControllerByIndex(i);
        const bool handheld_connected = (i == 0 && handheld->IsConnected());
        if (!controller->IsConnected() && !handheld_connected) {
            continue;
        }
        ++connected;
        const auto type = handheld_connected ? Core::HID::NpadStyleIndex::Handheld
                                             : controller->GetNpadStyleIndex();
        if (!IsControllerCompatible(type, parameters)) {
            all_compatible = false;
        }
    }

    return connected >= min_supported_players && connected <= max_supported_players &&
           all_compatible;
}

void QtControllerSelectorDialog::RefreshAndMaybeAccept() {
    auto& hid_core = system.HIDCore();
    const auto* handheld = hid_core.GetEmulatedController(Core::HID::NpadIdType::Handheld);
    ...
    for (std::size_t i = 0; i < NUM_PLAYERS; ++i) {
        const auto* controller = hid_core.GetEmulatedControllerByIndex(i);
        const bool handheld_connected = (i == 0 && handheld->IsConnected());
        const bool is_connected = controller->IsConnected() || handheld_connected;
        if (is_connected) {
            ++connected;
            const auto type = handheld_connected ? Core::HID::NpadStyleIndex::Handheld
                                                 : controller->GetNpadStyleIndex();
        ...
        if (i < static_cast<std::size_t>(max_supported_players)) {
            player_labels[i]->show();
            player_labels[i]->setText(tr("Player %1\n%2")
                                          .arg(static_cast<int>(i) + 1)
                                          .arg(is_connected ? tr("Ready") : tr("Waiting")));
            player_labels[i]->setStyleSheet(
                is_connected ? QStringLiteral("color: #7ee787; font-size: 11pt;")
                             : QStringLiteral("color: #8b8b8b; font-size: 11pt;"));
        }
    }

    // The prompt is driven by the SAME predicate as acceptance, so the screen never says "Ready"
    // while silently refusing to resume.
    const bool met = connected >= min_supported_players &&
                     connected <= max_supported_players && all_compatible;

    QString prompt;
    if (connected < min_supported_players) {
        prompt = min_supported_players <= 1
                     ? tr("Connect a controller to continue")
                     : tr("Connect %1 controllers to continue (%2 connected)")
                           .arg(min_supported_players)
                           .arg(connected);
        prompt += QStringLiteral("\n\n");
        prompt += tr("This screen closes automatically once a controller is connected.");
    } else if (connected > max_supported_players) {
        prompt = tr("Too many controllers connected for this game");
    } else if (!all_compatible) {
        // The auto-mapper binds Pro Controllers; a game that requires a different controller type
        // cannot be satisfied automatically. Tell the user what to do instead of hanging forever on
        // "Connect a controller". (Auto-binding non-Pro types is a separate enhancement.)
        prompt = tr("This game needs a different controller type.\n"
                    "Quit the game, then open Advanced controller settings to configure it.");
    } else {
        prompt = tr("Ready");
    }
    ui->labelError->setText(prompt);

    if (met) {
        if (refresh_timer != nullptr) {
            refresh_timer->stop();
        }
        // Only accept once the dialog is actually on-screen; the constructor primes this once
        // before exec() (which handles the already-satisfied case on its own).
        if (isVisible()) {
            accept();
        }
    }
...
void QtControllerSelectorDialog::LoadConfiguration() {
    // Observe-only: do NOT enter manual configuration mode. The Steam auto-mapper keeps the
    // emulated controllers bound to real connected pads; entering config mode here would snapshot
    // a stale temporary state and revert the auto-mapper's work when the applet closes.
...
void QtControllerSelectorDialog::keyPressEvent(QKeyEvent* evt) {
    // The simplified view auto-resumes once the required controllers are connected, so the legacy
    // manual add/remove-player key handling is no longer needed. Default behavior keeps Esc mapped
    // to the "Quit game" reject role.
...
bool QtControllerSelectorDialog::CheckIfParametersMet() {
    ...
    if (num_connected_players < min_supported_players ||
        ...
        // The simplified view clears the button box, so the Ok button may not exist; guard it.
        if (auto* const ok = ui->buttonBox->button(QDialogButtonBox::Ok)) {
            ok->setEnabled(parameters_met);
        }
