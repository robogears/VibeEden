...
public:
    ...
    /**
     * The Steam/SDL player index of this controller, i.e. the slot Steam assigns when
     * controllers are (re)ordered in its UI. Under Steam Input every virtual pad shares the
     * same GUID/name, so this index is the only stable way to tell them apart and to honor
     * the user's chosen Player 1..N order. Returns -1 when unknown (e.g. not running under
     * Steam Input), in which case callers should fall back to connection order.
     */
    int GetPlayerIndex() const {
        if (sdl_controller) {
            return SDL_GetGamepadPlayerIndex(sdl_controller.get());
        }
        return -1;
    }

    /**
     * Whether this pad is a Steam Input virtual controller (the "Steam Virtual Gamepad" Steam
     * presents in Big Picture / on the Steam Deck in Game Mode). Steam Input is the source of
     * truth for controller order on the Deck, and when it is active it is the *only* thing the
     * app should bind to - never the raw physical device. SDL exposes a non-zero Steam Input
     * handle for such controllers; we fall back to a name match for safety.
     */
    bool IsSteamVirtual() const {
        if (!sdl_controller) {
            return false;
        }
        if (SDL_GetGamepadSteamHandle(sdl_controller.get()) != 0) {
            return true;
        }
        const auto* const raw_name = SDL_GetGamepadName(sdl_controller.get());
        if (raw_name != nullptr) {
            return std::strstr(raw_name, "Steam Virtual Gamepad") != nullptr ||
                   std::strstr(raw_name, "Steam Controller") != nullptr;
        }
        return false;
    }
...
std::vector<Common::ParamPackage> SDLDriver::GetInputDevices() const {
    ...
    // The SDL watch thread mutates joystick_map on hotplug; every other accessor locks this mutex,
    // and the Steam auto-mapper now polls this every second, so guard the iteration against a
    // concurrent insert/erase that would otherwise invalidate iterators (crash).
    std::scoped_lock lock{joystick_map_mutex};
    ...
    for (const auto& [key, value] : joystick_map) {
        for (const auto& joystick : value) {
            ...
            devices.emplace_back(Common::ParamPackage{
                ...
                {"player_index", std::to_string(joystick->GetPlayerIndex())},
                {"steam_virtual", joystick->IsSteamVirtual() ? "1" : "0"},
            });
